@section('script')
    ...
    <script>
        ...
        $('#update-product').parsley();
        $('body').on('submit', '#update-product', function (e) {
            ...
            if ($('#update-product').parsley().isValid()) {
                $.ajax({
        ...
        function makeVariations(key, value) {
            ...
            $.each(value, function (k, v) {
                variantNames +=
                    '<div class="col-md-4">' +
                        '<input type="hidden" name="variants['+key+'][values][]" value="'+v.id+'">' +
            ...
            '<div class="row border mb-2">' +
                ...
                '<div class="col-md-3">' +
                    '<div class="form-group">' +
                        '<label>Price</label>' +
                        '<input name="variants['+key+'][price]" type="number" class="form-control" value="0" placeholder="price">' +
                    '</div>' +
                '</div>' +
                '<div class="col-md-3">' +
                    '<div class="form-group">' +
                        '<label>SKU</label>' +
                        '<input name="variants['+key+'][sku]" type="text" class="form-control" placeholder="SKU">' +
                    '</div>' +
                '</div>' +
                '<div class="col-md-3">' +
                    '<div class="form-group">' +
                        '<label>Stock</label>' +
                        '<input name="variants['+key+'][stock]" type="number" class="form-control" value="0" placeholder="Stock">' +
                    '</div>' +
                '</div>' +
                '<div class="col-md-3">' +
                    '<div class="form-group">' +
                        '<label>Status</label>' +
                        '<select name="variants['+key+'][status]" class="form-control">' +
